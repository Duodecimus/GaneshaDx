...
namespace GaneshaDx.Common {
	internal class GenerateTerrain {
			...
			foreach (var (polygontype, polylist) in CurrentMapState.StateData.PolygonCollection[MeshType.PrimaryMesh]) {
				foreach (Polygon poly in polylist) {
					...
					foreach (Vertex vert in poly.Vertices) {
						if (Math.Abs(vert.Position.X) > farthestX) { farthestX = Math.Abs((int)vert.Position.X); }
						if (Math.Abs(vert.Position.Z) > farthestZ) { farthestZ = Math.Abs((int)vert.Position.Z); }
				...
				for (int indexX = 0; indexX < CurrentMapState.StateData.Terrain.SizeX; indexX++) {
					...
					if (polyList != null) {
						...
						if (lowerTile.Count > 0) {
							...
							if (upperMaxY == 0 && upperMinY == 0 && lowerMaxY ==0 && lowerMinY == 0) { // Set lower to unselectable, and upper to impassable
								impassable0   = true;
								unselectable0 = false;
								impassable1   = false;
								unselectable1 = true;
							}
							else if (lowerMaxY == 0 && lowerMinY == 0) { // Set lower to unselectable
								impassable1   = false;
								unselectable1 = true;
								impassable0   = false;
								unselectable0 = false;
							} else { // Set both to normal terrain
								impassable1   = false;
								unselectable1 = false;
								impassable0   = false;
								unselectable0 = false;
		...
		public static (List<Vector3> upperGroup, List<Vector3> lowerGroup) SortAndGroupByY(List<Vector3> vertices) {
			...
			if (upperGroup.Count < 4) { //Same for the upper group
				upperGroup.Clear();
			}
		...
		private static (TerrainSlopeType slopeType, float minY, float maxY) DetermineSlopeType(List<Vector3> vertices, int centerX, int centerZ) {
			...
			if (vertices.Count <= 3) { return (TerrainSlopeType.Flat, 0, 0); }
			...
			if (yNorthEast== yNorthWest && yNorthEast == ySouthEast) {
				if (ySouthWest > yNorthEast) { return (TerrainSlopeType.ConvexNortheast, minY, maxY); }
				else { return (TerrainSlopeType.ConcaveSouthwest, minYCorner, maxYCorner); }
			}
			if (yNorthWest == yNorthEast && yNorthWest == ySouthWest) {
				if (ySouthEast > yNorthWest) { return (TerrainSlopeType.ConvexNorthwest, minY, maxY); }
				else { return (TerrainSlopeType.ConcaveSoutheast, minYCorner, maxYCorner); }
			}
			if (ySouthEast == ySouthWest && ySouthEast == yNorthEast) {
				if (yNorthWest > ySouthEast) { return (TerrainSlopeType.ConvexSoutheast, minY, maxY); }
				else { return (TerrainSlopeType.ConcaveNorthwest, minYCorner, maxYCorner); }
			}
			if (ySouthWest == ySouthEast && ySouthWest == yNorthWest) {
				if (yNorthEast > ySouthWest) { return (TerrainSlopeType.ConvexSouthwest, minY, maxY); }
				else { return (TerrainSlopeType.ConcaveNortheast, minYCorner, maxYCorner); }
			}
