...
public class TerrainTile {
	...
	public Vector3 AveragePoint {
		get {
			List<Vector3> adjustedVerts = new();
	...
	private void SetRenderVertices() {
		Vertices = new List<Vector3> {
			new(-IndexX * 28, (Height + Depth) * 12 + 1, IndexZ * 28),
			new(-IndexX * 28, (Height + Depth) * 12 + 1, IndexZ * 28 + 28),
			new(-IndexX * 28 - 28, (Height + Depth) * 12 + 1, IndexZ * 28 + 28),
			new(-IndexX * 28 - 28, (Height + Depth) * 12 + 1, IndexZ * 28),
		};

		List<TerrainSlopeType> vert0LiftTypes = new() {
		...
		List<TerrainSlopeType> vert1LiftTypes = new() {
		...
		List<TerrainSlopeType> vert2LiftTypes = new() {
			TerrainSlopeType.InclineNorth,
		...
		List<TerrainSlopeType> vert3LiftTypes = new() {
			TerrainSlopeType.InclineEast,
			TerrainSlopeType.InclineSouth,
		...
		Vector3 liftAmount = new(0, 12 + (SlopeHeight - 1) * 12, 0);
